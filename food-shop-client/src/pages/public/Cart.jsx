...
function Cart() {
    ...
    if (loading) return <main className="min-h-screen bg-slate-50 p-4 sm:p-8"><div className="mx-auto h-64 max-w-5xl animate-pulse rounded-[2rem] bg-white shadow-sm" /></main>;
    if (!items.length) return <main className="min-h-screen bg-[radial-gradient(circle_at_top,_#ffedd5,_transparent_42%),#f8fafc] px-4 py-20 text-center"><div className="mx-auto max-w-md rounded-[2rem] border border-orange-100 bg-white p-8 shadow-xl shadow-orange-950/5"><span className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-orange-100"><ShoppingBag className="text-orange-500" size={38} /></span><h1 className="mt-6 text-3xl font-black tracking-tight">Your cart is empty</h1><p className="mt-2 text-slate-500">Add something delicious from the menu.</p><Link to="/menu" className="mt-7 inline-block rounded-xl bg-slate-900 px-6 py-3 font-bold text-white shadow-lg shadow-slate-950/15 hover:bg-orange-600">Browse menu</Link></div></main>;
