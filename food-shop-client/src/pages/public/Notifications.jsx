...
function Notifications() {
    ...
    useEffect(() => { load(); }, []);
    const notifyNavbar = () => window.dispatchEvent(new Event("notifications-changed"));
    const read = async id => { try { await markNotificationRead(id); setItems(current => current.map(item => item._id === id ? { ...item, isRead: true } : item)); notifyNavbar(); } catch (error) { toast.error(message(error)); } };
    const readAll = async () => { try { await markAllNotificationsRead(); setItems(current => current.map(item => ({ ...item, isRead: true }))); notifyNavbar(); toast.success("All notifications marked as read."); } catch (error) { toast.error(message(error)); } };
    const remove = async id => { if (!window.confirm("Delete this notification?")) return; setDeleting(id); try { await deleteNotification(id); setItems(current => current.filter(item => item._id !== id)); notifyNavbar(); toast.success("Notification deleted."); } catch (error) { toast.error(message(error)); } finally { setDeleting(""); } };
